...
class Scene1(Entity):
    def __init__(self):
        ...
        self.enemies = []
        self.create_enemies()
        self.healthbar = HealthBar(color = color.red, max_value=3)
    def create_enemies(self):
        ...
        invoke(self.create_enemies, delay = 2)
...
class Retry(Button):
    def __init__(self):
        ...
        self.scale = (0.1,0.1)
def restart():
    scene1.healthbar.value = 3
    scene1.enabled = True
    retry.position = (10,10)
def update():
    if type(scene1.char.intersects().entity) == Enemy:
        scene1.healthbar.value -= 1
        hit = scene1.char.intersects().entity
        if scene1.healthbar.value > 0:
            destroy(scene1.char.intersects().entity)
            scene1.enemies.remove(hit)
        elif scene1.healthbar.value == 0:
